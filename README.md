...
| `BEAT_TOKEN` | when set, `/beat/{id}` requires `Authorization: Bearer <token>` from senders. Leaving it unset leaves the endpoint open; setting it to an empty or whitespace-only value fails startup instead. `BEAT_TOKEN_FILE` points at a mounted secret file instead | _(unset)_ | No |
...
A malformed `BEATS` or `DISCORD_WEBHOOK_URL` fails startup rather than falling back, and so does a webhook URL on any scheme other than `https` or a `NODE_NAME` over 256 bytes. A `BEAT_TOKEN` that is set but empty, or that contains nothing but spaces and tabs, fails startup too: HTTP strips the leading and trailing spaces and tabs from a header value, so no sender could ever present such a token, and knell would reject every ping while reporting itself gated. Unset the variable to serve `/beat/{id}` open on purpose. When a `_FILE` variable is set but its file cannot be read, because it is missing, unreadable, or empty, startup fails instead of falling back to the plain variable; only an unset `_FILE` variable falls back. A dead-man switch running with the wrong config is worse than one that refuses to start.

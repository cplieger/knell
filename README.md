...
| `BEAT_TOKEN` | when set, `/beat/{id}` requires `Authorization: Bearer <token>` from senders. Leaving it unset leaves the endpoint open. The token is verified exactly as configured and is never rewritten, so a value senders could not present as written — empty, carrying surrounding spaces or tabs, or carrying a control character HTTP forbids in a header — fails startup instead. `BEAT_TOKEN_FILE` points at a mounted secret file instead | _(unset)_ | No |
| `LISTEN_ADDR` | TCP listen address (`host:port`) | `:9190` | No |
| `ALLOWED_HOSTS` | comma-separated exact-match `Host` allowlist (bare hostnames or IPs, an optional port, no scheme or path), e.g. `knell.internal,10.0.0.5`. Unset accepts every `Host`; setting it rejects any other `Host` with 403 `host_not_allowed`, which is what blocks DNS rebinding from a browser inside your network. Loopback callers are always exempt, so the container healthcheck keeps working. Malformed entries are warned about and dropped, and an allowlist with no usable entry rejects every non-loopback request | _(unset)_ | No |

...
| `BEAT_TOKEN` | when set, `/beat/{id}` requires `Authorization: Bearer <token>` from senders. Leaving it unset leaves the endpoint open; setting it to a value no sender could present — empty, whitespace-only, or carrying a control character HTTP forbids in a header — fails startup instead. Surrounding spaces and tabs are trimmed, because HTTP strips them anyway. `BEAT_TOKEN_FILE` points at a mounted secret file instead | _(unset)_ | No |
...
A malformed `BEATS` or `DISCORD_WEBHOOK_URL` fails startup rather than falling back, and so does a webhook URL on any scheme other than `https` or a `NODE_NAME` over 256 bytes. A `BEAT_TOKEN` fails startup whenever no sender could present it: when it is set but empty, when it contains nothing but spaces and tabs, or when it still carries a control character after the surrounding spaces and tabs are trimmed. HTTP strips the leading and trailing spaces and tabs from a header value and forbids control characters in it outright, so knell would reject every ping while reporting itself gated, then declare every beat missing one deadline later. Surrounding spaces and tabs are trimmed rather than refused, so the token knell verifies is exactly the value that arrives on the wire; a non-ASCII space such as `U+00A0` does survive a header value, so a token containing one is accepted as configured. Unset the variable to serve `/beat/{id}` open on purpose. When a `_FILE` variable is set but its file cannot be read, because it is missing, unreadable, or empty, startup fails instead of falling back to the plain variable; only an unset `_FILE` variable falls back. A dead-man switch running with the wrong config is worse than one that refuses to start.
